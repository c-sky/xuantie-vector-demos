...
// trans input to stride 
	...
	uint16x2xm4_t *a0 = (uint16x2xm4_t *)input1;
        const unsigned short int *a1 = (const unsigned short int *)&input0[nstride[vlmax-1]];
        uint16xm4_t *a7 = (uint16xm4_t *)stride;
	uint16xm4_t *a8 = (uint16xm4_t *)input3;
        e16xm4_t a3;
	unsigned short int *aa;

        for(int t=0; t<N; t++) {
                gvl = vsetvli(avl[t], RVV_E16, RVV_M4);
		a3 = vmsnevi_e16xm4_uint16xm4(*a8, 0, gvl);
                bb=vlxseg2huv_mask_uint16x2xm4_uint16xm4(*a0, a1, *a7, a3, gvl);
