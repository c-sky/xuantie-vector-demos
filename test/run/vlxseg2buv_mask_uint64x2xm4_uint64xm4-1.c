...
// trans input to stride 
	...
	uint64x2xm4_t *a0 = (uint64x2xm4_t *)input1;
        const unsigned long long *a1 = (const unsigned long long *)&input0[nstride[vlmax-1]];
        uint64xm4_t *a7 = (uint64xm4_t *)stride;
	uint64xm4_t *a8 = (uint64xm4_t *)input3;
        e64xm4_t a3;
	unsigned long long *aa;

        for(int t=0; t<N; t++) {
                gvl = vsetvli(avl[t], RVV_E64, RVV_M4);
		a3 = vmsnevi_e64xm4_uint64xm4(*a8, 0, gvl);
                bb=vlxseg2buv_mask_uint64x2xm4_uint64xm4(*a0, a1, *a7, a3, gvl);
